...
<div>
    <form method="POST">
        <hr><h3 >Driver Registration</h3><hr>
        <table >
            <tr>
                <td>Name</td>
...
if (isset($_REQUEST['btnSubmit'])) {
    ...
    if ($r[0] > 0)    //to check whether the username exist
    {
        ...
        $q = "insert into driver (name,address,email,phone,district) values('$name','$address','$email','$contact','$dist')";
        $s = mysqli_query($conn, $q);
        if ($s) {

            $q = "insert into login (username,password,usertype,status) values('$email','$pwd','driver','1')";
            $s = mysqli_query($conn, $q);
            if ($s) {
